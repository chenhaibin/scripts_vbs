...
Const EXEFileToCheck = "C:\Program Files\ViewApps\VMM\TFXFrm32.ex_"
strCmd = "cmd.exe /c ""cscript //nologo " & wscript.scriptfullname & """ /t:"
strHeader = "Computer" & VBTAB & "WMI Query Result" & VBTAB & "OSQuery" & VBTAB & "TFXFrm32.ex_ Exists" & VBTAB & "TFXFrm32.ex_ Version" & VBTAB & "Last Logged On User" 'Log file Header Line
Const strLogFile = "C:\Matt\VMPluginReport.txt" 'Log File Location
Const rndOffsetUpper = 1000 'Maximum size of random offset for file write
...
If tmpTarget <> "" Then
	strTMP = TargetInfo(tmpTarget, EXEFileToCheck)
...
End Function
'*********************************TargetInfo
Private Function TargetInfo(strComputer, EXEFileToCheck)
On Error Resume Next
Dim OSDetected, arrResult, strLine
	strLine = strComputer
	OSDetected = OSQuery(strComputer)
	If OSDetected = "Not Found" Then
	strLine = strLine & VBTAB & "Unsuccessful" & vbtab & "Not Found" & vbtab & "NA" & vbtab & "NA"
	Else
	strLine = strLine & VBTAB & "Successful" & vbtab & OSDetected
